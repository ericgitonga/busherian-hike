...
export default function CheckinPage() {
  ...
  useEffect(() => {
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker
        .register("/sw-checkin.js", { scope: "/checkin" })
        .catch(() => {});
    }
  }, []);
  ...
  const handleScan = useCallback((decodedText: string) => {
    ...
    if (result.status === "already-checked-in") {
      setBanner({
        kind: "warning",
        text: `${result.attendee.name} is already checked in.`,
      });
      return;
    }

    const updated = current.map((a) =>
      a.id === result.attendee.id ? { ...a, checkedIn: true } : a,
    );
    setAttendees(updated);
    localStorage.setItem(ATTENDEES_KEY, JSON.stringify(updated));

    const pending = [...stateRef.current.pendingSyncs, { registrationId: result.attendee.id, checkedInAt: Date.now() }];
